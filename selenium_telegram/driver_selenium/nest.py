...
class HydroTelegramTokenizer922(Tokenizer):
    patterns = [
        ...
        r'(?P<GROUP922_2>2\d+)',
        r'(?P<GROUP922_3>3\d+)',
        r'(?P<GROUP922_4>4\d{4}|4\d{2}//|4////|4//d{2})',
        r'(?P<GROUP922_5>5\d+)',
        r'(?P<GROUP922_6>6\d+)',
        r'(?P<GROUP922_7>7\d+)',
        r'(?P<GROUP922_8>8\d+)',
...
class ObsTimeDecoder(AbstractDecoder):
    def decode(self, value):
        return self.create_dict('value', value, 'day', value[:2], 'hour', value[2:4], 'telegram_section_indicator',
                                value[4])
...
class WaterLevelDecoder(AbstractDecoder):
    ...
    def decode(self, value):
        ...
        try:
            match value[0]:
                case '1':
                    return self.create_dict('value', decod_value, 'unit', 'cm')
                case '3':
                    return self.create_dict('value', decod_value,
...
class TemperatureDecoder(AbstractDecoder):
    ...
    def value_validator(self, value):
        ...
        validator_water = value[1:3].isdigit()
        validator_air =  value[3:].isdigit()
        if validator_air == False:
            return {'value': value[1:],
                    'water_temperature': self._decode_value_water(value),
                    'air_temperature': None, 'unit': 'Cel'}
        elif validator_water == False:
            return {'value': value[1:],
    ...
    def _decode_value_air(self, value):
        try:
            match value[3]:
                case '5':
                    return -(float(int(value[4])))
                case '6':
                    return -(float(int(value[4]) + 10))
                case '7':
                    return -(float(int(value[4]) + 20))
                case '8':
                    return -(float(int(value[4]) + 30))
                case '9':
                    return -(float(int(value[4]) + 40))
                case _:
                    return float(int(value[3:]))
        except:
            return None
            ...
            if data is not None:
                ...
                if data['intensity']:
                    if intensity is not None:
                        ...
                        result_dict[
                            'error'] = f"Error: Phenomenon {key} requires intensity but none provided in value {value}"
                return result_dict
            else:
                data = self.data_dict.get(key)
...
class IcePhenomenaDecoder(BaseConditionDecoder):
    data_dict = {
        ...
        '14': {'intensity': False, 'description_en': 'Ice floe with a width over 100m',
               'description_uk': 'Припай шириною більше 100м'},
        '15': {'intensity': False, 'description_en': 'Overhanging ice on the shore',
               'description_uk': 'Забереги навислі'},
        ...
        '18': {'intensity': False, 'description_en': 'Icebreaker over the ice cover',
               'description_uk': 'Льодохід поверх льдового покриву'},
        ...
        '23': {'intensity': False, 'description_en': 'Ice avalanches on the shore',
               'description_uk': 'Навали льоду на березі'},
        '24': {'intensity': False, 'description_en': 'Ice dam in the post area',
               'description_uk': 'Льодова перемичка у створі поста'},
        '25': {'intensity': False, 'description_en': 'Ice dam above the post',
               'description_uk': 'Льодова перемичка вище поста'},
        '26': {'intensity': False, 'description_en': 'Ice dam below the post',
               'description_uk': 'Льодова перемичка нижче поста'},
        '30': {'intensity': False, 'description_en': 'Ice jam above the post',
               'description_uk': 'Затор льоду вище поста'},
        '31': {'intensity': False, 'description_en': 'Ice jam below the post',
               'description_uk': 'Затор льоду нижче поста'},
        '32': {'intensity': False, 'description_en': 'Ice jam artificially destroyed',
               'description_uk': 'Затор льоду штучно руйнується'},
        '34': {'intensity': False, 'description_en': 'Ice jam above the station',
               'description_uk': 'Зажор льоду вище поста'},
        '35': {'intensity': False, 'description_en': 'Ice jam below the station',
               'description_uk': 'Зажор льоду нижче поста'},
        '36': {'intensity': False, 'description_en': 'Ice jam artificially destroyed',
               'description_uk': 'Зажор льоду штучно руйнується'},
        '37': {'intensity': False, 'description_en': 'Water on the ice', 'description_uk': 'Вода на льоду'},
        '38': {'intensity': False, 'description_en': 'Water flowing on the ice',
               'description_uk': 'Вода тече поверх льоду'},
        ...
        '46': {'intensity': True, 'description_en': 'Residual ice on the shore',
               'description_uk': 'Забереги залишкові'},
        ...
        '53': {'intensity': False, 'description_en': 'Ice moving away from the shore',
               'description_uk': 'Лід відносить від берега'},
        '54': {'intensity': False, 'description_en': 'Ice pressed against the shore',
               'description_uk': 'Лід притиснуло до берега'},
        '63': {'intensity': True, 'description_en': 'Incomplete ice cover', 'description_uk': 'Льодостав неповний'},
        '64': {'intensity': True, 'description_en': 'Ice cover with polynyas',
               'description_uk': 'Льодовий покрив з ополонками'},
        ...
        '67': {'intensity': False, 'description_en': 'Ice cover with ridges of floes',
               'description_uk': 'Льодовий покрив з грядами торосів'},
        ...
        '70': {'intensity': False, 'description_en': 'Cracks in the ice cover',
               'description_uk': 'Тріщини у льодовому покриві'},
        ...
        '76': {'intensity': False, 'description_en': 'Ice artificially destroyed',
               'description_uk': 'Лід штучно зруйновано'},
...
class WaterBodyConditionDecoder(BaseConditionDecoder):
    data_dict = {
        ...
        "14": {'intensity': False, 'description_en': 'Forest bend above the station',
               'description_uk': 'Залом лісу вище поста'},
        "15": {'intensity': False, 'description_en': 'Forest bend below the station',
               'description_uk': 'Залом лісу нижче поста'},
        "22": {'intensity': True, 'description_en': 'Vegetation near the shore',
               'description_uk': 'Рослинність біля берега'},
        "23": {'intensity': True, 'description_en': 'Vegetation across the river section',
               'description_uk': 'Рослинність по всьому перерізу річки'},
        "24": {'intensity': True, 'description_en': 'Vegetation in patches across the stream section',
               'description_uk': 'Рослинність по перерізу потоку плямами'},
        "25": {'intensity': False, 'description_en': 'Vegetation spreading along the bottom',
               'description_uk': 'Рослинність стелеться по дну'},
        "26": {'intensity': False, 'description_en': 'Vegetation mowed in the water column',
               'description_uk': 'Рослинність у гідростворі викошена'},
        "27": {'intensity': False, 'description_en': 'Vegetation lying on the bottom',
               'description_uk': 'Рослинність лягла на дно'},
        "28": {'intensity': False, 'description_en': 'Vegetation covered with silt',
               'description_uk': 'Рослинність занесено мулом'},
        "29": {'intensity': False, 'description_en': 'Vegetation died due to river pollution',
               'description_uk': 'Рослинність загинула в результаті забруднення річки'},
        "35": {'intensity': False, 'description_en': 'Bank collapse (erosion) in the station pool',
               'description_uk': 'Обвал (зсув) берега у сворі поста'},
        "36": {'intensity': False, 'description_en': 'Bank collapse (erosion) above the station',
               'description_uk': 'Обвал (зсув) берега вище поста'},
        "37": {'intensity': False, 'description_en': 'Bank collapse (erosion) below the station',
               'description_uk': 'Обвал (зсув) берега нижче поста'},
        "38": {'intensity': False, 'description_en': 'Channel dredging works',
               'description_uk': 'Днопоглиблювальні роботи в руслі'},
        "39": {'intensity': False, 'description_en': 'Channel embankment works',
               'description_uk': 'Намивні роботи в руслі'},
        "40": {'intensity': False, 'description_en': 'Channel cleared', 'description_uk': 'Русло розчищено'},
        "41": {'intensity': False,
               'description_en': 'River channel narrowed in the water column for water flow measurement',
               'description_uk': 'Русло річки звужено в гідростворі для вимірювання витрат води'},
        ...
        "48": {'intensity': False, 'description_en': 'Shift in the channel in the plan',
               'description_uk': 'Зміщення русла у плані'},
        "52": {'intensity': False, 'description_en': 'Channel overgrown with reeds',
               'description_uk': 'Русло заросло очеретом'},
        "53": {'intensity': True, 'description_en': 'Reeds in the channel', 'description_uk': 'Очерет у руслі'},
        "54": {'intensity': True, 'description_en': 'Reeds in the channel above the station',
               'description_uk': 'Очерет у руслі вище поста'},
        "55": {'intensity': True, 'description_en': 'Reeds in the channel below the station',
               'description_uk': 'Очерет у руслі нижче поста'},
        "56": {'intensity': False, 'description_en': 'Debris flow passage',
               'description_uk': 'Проходження селевого потоку'},
        "57": {'intensity': False, 'description_en': 'River flow changed to opposite direction',
               'description_uk': 'Течія річки змінилася на протилежну'},
        ...
        "61": {'intensity': False, 'description_en': 'Weak waves (1 point)',
               'description_uk': 'Хвилювання слабке (1 бал)'},
        "62": {'intensity': False, 'description_en': 'Moderate waves (2-3 points)',
               'description_uk': 'Хвилювання помірне (2-3 бали)'},
        "63": {'intensity': False, 'description_en': 'Strong waves (more than 4 points)',
               'description_uk': 'Хвилювання сильне (більше 4 балів)'},
        "64": {'intensity': False, 'description_en': 'Stagnant water', 'description_uk': 'Стояча вода'},
        "65": {'intensity': False, 'description_en': 'Stagnant water under the ice',
               'description_uk': 'Стояча вода під льодом'},
        "66": {'intensity': False, 'description_en': 'Boat crossing stopped',
               'description_uk': 'Припинилась переправа на човнах'},
        "67": {'intensity': False, 'description_en': 'Pedestrian traffic on ice stopped',
               'description_uk': 'Припинилося пішоходне сполучення по льоду'},
        "68": {'intensity': False, 'description_en': 'Pedestrian traffic on ice started',
               'description_uk': 'Розпочалося пішоходне сполучення по льоду'},
        "69": {'intensity': False, 'description_en': 'Transportation on ice started',
               'description_uk': 'Розпочався рух транспорту на льоду'},
        "70": {'intensity': False, 'description_en': 'Transportation on ice stopped',
               'description_uk': 'Припинився рух транспорту на льоду'},
        "71": {'intensity': False, 'description_en': 'Boat crossing started',
               'description_uk': 'Розпочалася переправа човнами'},
        "72": {'intensity': False, 'description_en': 'Shore support from the river, reservoir, lake',
               'description_uk': 'Підпір від річки, водосховища, озера'},
        ...
        "77": {'intensity': False, 'description_en': 'Water intake above the station',
               'description_uk': 'Забір води вище поста'},
        "78": {'intensity': False, 'description_en': 'Water intake below the station',
               'description_uk': 'Забір води нижче поста'},
        "79": {'intensity': False, 'description_en': 'Water intake above the station stopped',
               'description_uk': 'Забір води вище поста припинився'},
        "80": {'intensity': False, 'description_en': 'Water intake below the station stopped',
               'description_uk': 'Забір води нижче поста припинився'},
        "81": {'intensity': False, 'description_en': 'Water discharge above the station',
               'description_uk': 'Скид води вище поста'},
        "82": {'intensity': False, 'description_en': 'Water discharge below the station',
               'description_uk': 'Скид води нижче поста'},
        "83": {'intensity': False, 'description_en': 'Water discharge above the station stopped',
               'description_uk': 'Скид води вище поста припинився'},
        "84": {'intensity': False, 'description_en': 'Water discharge below the station stopped',
               'description_uk': 'Скид води нижче поста припинився'},
        "85": {'intensity': False, 'description_en': 'Dam (weir) above the station',
               'description_uk': 'Гребля (перетинка, загата, дамба) вище поста'},
        "86": {'intensity': False, 'description_en': 'Dam (weir) below the station',
               'description_uk': 'Гребля (перетинка, загата, дамба) нижче поста'},
        "87": {'intensity': False, 'description_en': 'Dam (weir) above the station destroyed',
               'description_uk': 'Зруйнована гребля (перетинка, загата, дамба) вище поста'},
        "88": {'intensity': False, 'description_en': 'Dam (weir) below the station destroyed',
               'description_uk': 'Зруйнована гребля (перетинка, загата, дамба) нижче поста'},
        "89": {'intensity': False, 'description_en': 'Channel support from debris obstruction',
               'description_uk': 'Підпір від засмічення (захаращення) русла'},
        "90": {'intensity': False, 'description_en': 'Channel support from bridge crossings',
               'description_uk': 'Підпір від мостових переправ'},
        "91": {'intensity': False, 'description_en': 'Water discharge from the reservoir',
               'description_uk': 'Пропуски води з водосховища'}
    }
...
class DecoderIntegerPart:
    def decode_value(self, value):
        try:
            match int(value[1]):
                case 0:
                    return float(int(value[2:5]) / 1000)
                case 1:
                    return float(int(value[2:5]) / 100)
                case 2:
                    return float((int(value[2:5])) / 10)
                case 3:
...
data_telegram = {'id_teleg': '8180120240101080000', 'date_telegram': '2024-01-01',
       'time_telegram': '08:00:00', 'index_station': '81801',
       'gauges_telegram': '81801 01081 10350 20021 30360 4//22 82234 00000='}
